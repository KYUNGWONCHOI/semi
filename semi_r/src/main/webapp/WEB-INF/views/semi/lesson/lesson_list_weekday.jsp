...
<form id="frm-lessonday">
	<div class="lessonday grid">
	<c:choose>
		<c:when test="${empty listLessonWeekday }">
		수업 없음
		</c:when>
		<c:otherwise>
			<div></div>
			...
			<c:forEach items="${listLessonWeekday }" var="vo" varStatus="vs">
				<c:choose>
	...
	<div>
		<button type="button" class="btn bookday">예약하기</button>
	</div>
</form>
<script>
	...
	$(loadedHandler);
		function loadedHandler(){
			//event 등록
			$(".btn.bookday").on("click", btnDayBookClickHandler);
	}
	function btnDayBookClickHandler(){
		if(checkLogin("로그인되어야 예약이 가능합니다.\n로그인페이지로 이동하시겠습니까?","bookWeekdayClass")){
			return false;
		}	
		$.ajax({
			async : false,
		...
		location.href="${pageContext.request.contextPath}/notice?tabmenu=3";
	}
